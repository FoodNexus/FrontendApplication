...
import { DonorLotsService } from './donor-lots.service';
import { NutriflowRecyclerRequestsService } from './nutriflow-recycler-requests.service';
...
export class NutriflowHubSyncService {
  ...
  private readonly donorLots = inject(DonorLotsService);
  private readonly recyclerRequests = inject(NutriflowRecyclerRequestsService);
  ...
  private start(base: string): void {
    ...
    window.addEventListener(NutriflowRecyclerRequestsService.CHANGED_EVENT, schedulePush);
    window.addEventListener(DonorLotsService.MUTATED_EVENT, schedulePush);
    window.addEventListener(NUTRIFLOW_CREDITS_MUTATED_EVENT, schedulePush);

    this.destroyRef.onDestroy(() => {
      window.removeEventListener(NutriflowRecyclerRequestsService.CHANGED_EVENT, schedulePush);
      window.removeEventListener(DonorLotsService.MUTATED_EVENT, schedulePush);
  ...
  private push(base: string): Observable<void> {
    const lots = localStorage.getItem(DonorLotsService.STORAGE_KEY) ?? '[]';
    const req = localStorage.getItem(NutriflowRecyclerRequestsService.STORAGE_KEY) ?? '[]';
  ...
  private applyServerToLocal(data: { lots: string | null; req: string | null; cred: string | null }): void {
    if (data.lots != null && data.lots.trim() !== '') {
      try {
        ...
        if (parsed.length === 0) {
          const localLots = this.donorLots.getAll();
          if (localLots.length > 0) {
            // Ne pas écraser des lots donateurs locaux avec un tableau vide du hub.
          } else {
            localStorage.setItem(DonorLotsService.STORAGE_KEY, data.lots);
          }
        } else {
          localStorage.setItem(DonorLotsService.STORAGE_KEY, data.lots);
        }
  ...
  private applyRecyclerRequestsFromServer(serverRaw: string): void {
    ...
    const local = this.recyclerRequests.getAll();
    const merged = this.recyclerRequests.mergeWithRemote(local, remote);
    this.recyclerRequests.saveAll(merged);
  }
}
